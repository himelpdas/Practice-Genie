...
# db.patient.first_name.widget = SQLFORM.widgets.autocomplete(request, db.patient.first_name, limitby=(0, 10),
# min_length=0, distinct=True)
# db.patient.last_name.widget = SQLFORM.widgets.autocomplete(request, db.patient.last_name, limitby=(0, 10),
# min_length=0, distinct=True)
# db.patient.date_of_birth.widget = SQLFORM.widgets.autocomplete(request, db.patient.date_of_birth, limitby=(0, 10),
# min_length=0, distinct=True)  # according to the souce, at_beginning True uses field.like(<beginning>%),
# where as False uses field.contains(<any part>)


db.define_table(
    "site",
    Field('name'),
    Field('phone', requires=PHONE_VALIDATOR),
    Field('fax', requires=PHONE_VALIDATOR),
    Field('address'),
    Field('city'),
...
bucket_generic_fields = [
    Field('patient', 'reference patient',
          requires=IS_IN_DB(db, db.patient, '%(last_name)s, %(first_name)s'), readable=False, writable=False),
    ...
    Field('destination', "reference destination", requires=IS_IN_DB(db, db.destination, '%(name)s')),  # 1st arg can be db or query set: db.person.name.requires = IS_IN_DB(db(db.person.id>10), 'person.id', '%(name)s')
